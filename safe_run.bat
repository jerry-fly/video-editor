...
if %ERRORLEVEL% neq 0 (
    echo 警告: 依赖项检查失败，将尝试继续运行...
    echo 尝试离线安装...
    python install_dependencies.py --offline --no-proxy
)

echo.
echo 检查moviepy.editor模块...
python -c "import moviepy.editor" 2>nul
if %ERRORLEVEL% neq 0 (
    echo 警告: 未找到moviepy.editor模块，尝试修复...
    python fix_moviepy.py
    if %ERRORLEVEL% neq 0 (
        echo 警告: 修复moviepy.editor失败，将尝试继续运行...
    )
)
...
if %ERRORLEVEL% neq 0 (
    echo 警告: 安装psutil失败，将尝试继续运行...
)

echo.
echo 清理现有的VideoEditor进程...
python process_monitor.py --cleanup
if %ERRORLEVEL% neq 0 (
    echo 警告: 进程清理失败，将尝试继续运行...
)
